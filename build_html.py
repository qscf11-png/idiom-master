                ...
                return e('div', { className: "flex flex-col items-center justify-center min-h-screen p-4" },
                    e('div', { className: "glass max-w-md w-full rounded-[32px] shadow-2xl p-8" },
                        ...
                        e('div', { className: "space-y-6" },
                            e('div', null,
                                e('label', { className: "text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2" }, "Level 範圍"),
                                e('div', { className: "flex flex-wrap gap-2" },
                                    [1,2,3,4,5,6].map(l => e('button', {
                                        key: l,
                                        onClick: () => levels.includes(l) ? setLevels(levels.filter(x=>x!==l)) : setLevels([...levels, l]),
                                        className: `px-3 py-1.5 rounded-lg text-xs font-bold border-2 transition-all ${levels.includes(l)?'bg-indigo-600 border-indigo-600 text-white':'border-slate-100 text-slate-400'}`
                                    }, `L${l}`))
                                )
                            ),
                            e('div', { className: "grid grid-cols-2 gap-4" },
                                e('div', null,
                                    e('label', { className: "text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2" }, "首字母 (A-Z)"),
                                    e('select', { value: letter, onChange: e=>setLetter(e.target.value), className: "w-full bg-slate-50 rounded-xl p-2 text-sm font-bold border-none ring-1 ring-slate-200" },
                                        ['All', ...'abcdefghijklmnopqrstuvwxyz'.split('')].map(c => e('option', { key: c, value: c }, c.toUpperCase()))
                                    )
                                ),
                                e('div', null,
                                    e('label', { className: "text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2" }, "詞性篩選"),
                                    e('select', { value: posFilter, onChange: e=>setPosFilter(e.target.value), className: "w-full bg-slate-50 rounded-xl p-2 text-sm font-bold border-none ring-1 ring-slate-200" },
                            ...
                            e('div', null,
                                e('div', { className: "flex justify-between mb-2" },
                                    e('label', { className: "text-[10px] font-black text-slate-400 uppercase tracking-widest" }, "抽樣數量"),
                    ...
                    e('div', { 
                        ...
                        e('div', { className: "absolute inset-0 backface-hidden bg-white rounded-[40px] shadow-2xl p-8 flex flex-col rotate-y-180 border-8 border-white" },
                            ...
                            e('div', { className: "grid grid-cols-4 gap-2 mt-6" },
                                ...
                                e('button', { onClick: es => { es.stopPropagation(); handleSRS('easy'); }, className: "py-3 bg-green-50 text-green-600 rounded-2xl text-[9px] font-black" }, "EASY")
                            )
                        )
                    )
                ),
                e('div', { className: "w-full max-w-md flex items-center gap-4 py-2 shrink-0" },
                    e('i', { 'data-lucide': 'volume-1', className: "w-4 h-4 text-indigo-400" }),
                    e('input', { type: 'range', min: '0.5', max: '2.0', step: '0.1', value: speechRate, onChange: e=>setSpeechRate(parseFloat(e.target.value)), className: "flex-grow h-1 bg-white rounded-full appearance-none accent-indigo-600" }),
                    e('span', { className: "text-[10px] font-black text-indigo-600" }, speechRate.toFixed(1) + "x")
                )
            );
        };
